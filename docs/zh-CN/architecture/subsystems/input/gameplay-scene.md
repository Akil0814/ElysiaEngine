...
首次进入默认 WASD 控制玩家 1、方向键控制玩家 2，鼠标归 P1。未绑定手柄 Start 加入玩家 2，保留方向键控制。公共菜单可交换键盘方案、转移鼠标、重新分配手柄、独立指定 UI 手柄并测试文本捕获。进入时保存外部键盘／鼠标配置，退出恢复；手柄归属继续保留。示例只展示两个角色，底层注册表与控制器管理不限制两人。共享相机，不含分屏、联网或个人焦点树。
