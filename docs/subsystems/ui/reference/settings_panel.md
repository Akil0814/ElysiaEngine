...
表单使用显示、音频和通用三个页签；每个页面拥有独立的纵向滚动容器，标题、状态消息及 Save/Back 不随页面滚动。控制/输入映射页不属于当前预设，后续可以作为新页面加入。
...
- `reset_navigation_state` 只把页面切回显示页并将三个滚动位置归零，不修改草稿、候选项或回调。
...
窗口模式、帧率和语言三个 Dropdown 都需要窗口级 popup 注册，因此必须在面板已由对应 `UiWindow` 拥有后调用 `register_with_window`；换窗口或提前移除时调用 `unregister_from_window`。析构与 `reset` 会自动解除现有窗口注册。
