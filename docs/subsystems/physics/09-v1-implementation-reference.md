...
当构建启用 `ELYSIA_ENABLE_IMGUI` 时，三个 Physics Combat Demo 还会通过共同基类注册同一个 Physics Inspector。它只读取 `last_step_stats()`、`debug_snapshot()` 和 `PhysicsWorldConfig`，调试复选框直接修改既有 `DebugDraw` 分类；下一次 Scene update 仍由上述映射决定 `PhysicsDebugCapture`，Inspector 不参与物理模拟。生命周期和发布成本见 [Development Overlay](../development-overlay.md)。
