...
void UiTestScene::on_enter(const elysia::scene::ScenePayload& payload)
{
    ...
    if (!test_payload || !is_valid_return_route(test_payload->return_route))
        throw std::logic_error("UiTestScene requires DemoScenePayload with a valid return route.");
...
void UiTestScene::set_active_theme(UiBuiltinTheme theme)
{
    ...
    set_status_key("testbed.ui_test.status.interaction");
}
...
void UiTestScene::rebuild_ui()
{
    ...
    refresh_theme_preview_styles();
    auto status = std::make_unique<UiLabel>(elysia::core::Rect{ 0,0,780,30 },0,ui_text_key("testbed.ui_test.status.ready"));
    ...
    auto* overview_section = add_section(*overview_list,"testbed.ui_test.pages.overview","testbed.ui_test.sections.overview");
    auto primary = button("testbed.ui_test.actions.replay"); primary->set_visual_role(UiButtonVisualRole::Primary);
    primary->set_on_click([this]() { set_status_key("testbed.ui_test.status.interaction"); }); overview_section->add_back(std::move(primary));
    auto danger = button("testbed.ui_test.actions.reset"); danger->set_visual_role(UiButtonVisualRole::Danger);
    ...
    (void)tabs->add_tab(ui_text_key("testbed.ui_test.pages.overview"),std::move(overview));

    UiListContainer* controls_list = nullptr; auto controls = page_scroll(controls_list);
    auto* controls_section = add_section(*controls_list,"testbed.ui_test.pages.controls","testbed.ui_test.sections.controls");
    ...
    check->set_on_toggled([this](UiCheckboxState) { set_status_key("testbed.ui_test.status.interaction"); }); controls_section->add_back(std::move(check));
    UiLabeledCheckboxConfig check_config{}; check_config.text_content = ui_text_key("testbed.ui_test.controls.labeled_check"); controls_section->add_back(std::make_unique<UiLabeledCheckbox>(elysia::core::Rect{ 0,0,360,42 },check_config));
    auto radios = std::make_unique<UiRadioGroup>(elysia::core::Rect{ 0,0,360,130 }); radios->set_on_selection_changed([this](std::optional<std::size_t>) { set_status_key("testbed.ui_test.status.interaction"); });
    radios->add_back(std::make_unique<UiRadioButton>(elysia::core::Rect{ 0,0,42,42 })); UiLabeledRadioButtonConfig radio_config{}; radio_config.text_content = ui_text_key("testbed.ui_test.controls.labeled_radio"); radios->add_back(std::make_unique<UiLabeledRadioButton>(elysia::core::Rect{ 0,0,300,42 },radio_config)); controls_section->add_back(std::move(radios));
    auto slider = std::make_unique<UiSlider>(elysia::core::Rect{ 0,0,360,48 }); slider->set_value_display(UiSliderValueDisplay::Percent); slider->set_value(0.5f); slider->set_on_value_changed([this](float) { set_status_key("testbed.ui_test.status.interaction"); }); controls_section->add_back(std::move(slider));
    ...
    auto input = std::make_unique<UiTextInput>(elysia::core::Rect{ 0,0,360,44 }); input->set_placeholder_content(ui_text_key("testbed.ui_test.controls.placeholder")); input->set_max_length(16); input->set_on_submit([this](std::string_view) { set_status_key("testbed.ui_test.status.interaction"); }); controls_section->add_back(std::move(input));
    UiDragHandleConfig drag_config{}; drag_config.axis = UiDragAxis::Horizontal; drag_config.drag_bounds = elysia::core::Rect{ 0,0,400,50 }; auto drag = std::make_unique<UiDragHandle>(elysia::core::Rect{ 0,0,28,28 },drag_config); drag->set_on_dragged([this](const elysia::core::Vector2&) { set_status_key("testbed.ui_test.status.interaction"); }); controls_section->add_back(std::move(drag));
    (void)tabs->add_tab(ui_text_key("testbed.ui_test.pages.controls"),std::move(controls));

    UiListContainer* media_list = nullptr; auto media = page_scroll(media_list);
    auto* media_section = add_section(*media_list,"testbed.ui_test.pages.media","testbed.ui_test.sections.media");
    auto fade_label = std::make_unique<UiFadeLabel>(elysia::core::Rect{ 0,0,360,34 },0,ui_text_key("testbed.ui_test.actions.replay")); fade_label->configure_playback(effects::UiOpacityFadeMode::FadeInOut,0.0,0.25,0.25); fade_label->play(); media_section->add_back(std::move(fade_label));
    auto blink_label = std::make_unique<UiBlinkLabel>(elysia::core::Rect{ 0,0,360,34 },0,ui_text_key("testbed.ui_test.pages.media")); blink_label->configure_playback(effects::UiOpacityBlinkMode::VisibleFirst,0.0,0.2,0.2,2); blink_label->play(); media_section->add_back(std::move(blink_label));
    auto pulse_label = std::make_unique<UiPulseLabel>(elysia::core::Rect{ 0,0,360,34 },0,ui_text_key("testbed.ui_test.sections.media")); pulse_label->configure_playback(effects::UiOpacityPulseMode::MinToMax,0.0,0.25,0.25,2); pulse_label->play(); media_section->add_back(std::move(pulse_label));
    auto text = std::make_unique<UiTextBlock>(elysia::core::Rect{ 0,0,700,84 }); text->set_text_content(ui_text_key("testbed.ui_test.media.long_text")); text->set_padding(6); media_section->add_back(std::move(text));
    ...
    (void)tabs->add_tab(ui_text_key("testbed.ui_test.pages.media"),std::move(media));

    UiListContainer* containers_list = nullptr; auto containers = page_scroll(containers_list); auto* container_section = add_section(*containers_list,"testbed.ui_test.pages.containers","testbed.ui_test.sections.containers");
    ...
    auto chrome = std::make_unique<UiChromeContainer>(elysia::core::Rect{ 0,0,620,170 }); chrome->add_left_action(button("common.back")); chrome->add_title_child(std::make_unique<UiLabel>(elysia::core::Rect{ 0,0,240,30 },0,ui_text_key("testbed.ui_test.containers.chrome"))); chrome->add_right_action(button("common.close")); auto chrome_body = std::make_unique<UiPanel>(elysia::core::Rect{ 0,0,580,100 }); chrome_body->add_child(button("testbed.ui_test.actions.replay")); chrome->set_body(std::move(chrome_body)); container_section->add_back(std::move(chrome));
    auto nested_tabs = std::make_unique<UiTabContainer>(elysia::core::Rect{ 0,0,620,200 }); auto nested_page = std::make_unique<UiPanel>(elysia::core::Rect{ 0,0,580,140 }); nested_page->add_child(button("common.confirm")); (void)nested_tabs->add_tab(ui_text_key("testbed.ui_test.containers.nested_tab"),std::move(nested_page)); container_section->add_back(std::move(nested_tabs));
    ...
    (void)tabs->add_tab(ui_text_key("testbed.ui_test.pages.containers"),std::move(containers));

    UiListContainer* overlays_list = nullptr; auto overlays = page_scroll(overlays_list); auto* overlay_section = add_section(*overlays_list,"testbed.ui_test.pages.overlays","testbed.ui_test.sections.overlays");
    ...
    auto open_overlay = button("testbed.ui_test.overlays.open_overlay"); open_overlay->set_on_click([this,overlay_ptr]() { _root_window->open_overlay(*overlay_ptr); set_status_key("testbed.ui_test.status.interaction"); }); overlay_section->add_back(std::move(open_overlay));
    auto dialog = std::make_unique<UiDialog>(elysia::core::Rect{ 0,0,480,300 }); UiDialog* dialog_ptr = dialog.get(); dialog->set_title_content(ui_text_key("testbed.ui_test.dialog.title")); dialog->set_body_content(ui_text_key("testbed.ui_test.dialog.body")); dialog->set_action_content(ui_text_key("common.close")); _root_window->add_child(std::move(dialog),at(760,230,480,300)); (void)dialog_ptr->register_with_window(*_root_window); auto open_dialog = button("testbed.ui_test.overlays.open_dialog"); open_dialog->set_on_click([dialog_ptr]() { dialog_ptr->open(); }); overlay_section->add_back(std::move(open_dialog));
    auto confirm = std::make_unique<UiConfirmationDialog>(elysia::core::Rect{ 0,0,440,220 }); UiConfirmationDialog* confirm_ptr = confirm.get(); confirm->set_config(UiConfirmationDialogConfig{ .title=ui_text_key("testbed.ui_test.confirm.title"),.message=ui_text_key("testbed.ui_test.confirm.message"),.confirm=ui_text_key("common.confirm"),.cancel=ui_text_key("common.cancel"),.close=ui_text_key("common.close"),.confirm_visual_role=UiButtonVisualRole::Danger }); _root_window->add_child(std::move(confirm),at(760,230,440,220)); (void)confirm_ptr->register_with_window(*_root_window); auto open_confirm = button("testbed.ui_test.overlays.open_confirm"); open_confirm->set_on_click([confirm_ptr]() { confirm_ptr->open(); }); overlay_section->add_back(std::move(open_confirm));
    auto dropdown = std::make_unique<UiDropdown>(elysia::core::Rect{ 0,0,320,42 }); dropdown->set_options({ UiDropdownOption{ui_text_key("common.confirm")},UiDropdownOption{ui_text_key("common.cancel")},UiDropdownOption{ui_text_key("common.close")} }); dropdown->register_with_window(*_root_window); overlay_section->add_back(std::move(dropdown)); auto tooltip_trigger = button("testbed.ui_test.overlays.tooltip"); UiButton* tooltip_trigger_ptr = tooltip_trigger.get(); overlay_section->add_back(std::move(tooltip_trigger)); auto* tooltip = _root_window->create_child<UiTooltip>(0); auto tooltip_content = std::make_unique<UiTextBlock>(elysia::core::Rect{ 0,0,280,72 }); tooltip_content->set_text_content(ui_text_key("testbed.ui_test.overlays.tooltip_text")); tooltip_content->set_padding(8); tooltip->bind_trigger(*tooltip_trigger_ptr); tooltip->set_content(std::move(tooltip_content)); tooltip->register_with_window(*_root_window);
    (void)tabs->add_tab(ui_text_key("testbed.ui_test.pages.overlays"),std::move(overlays));

    UiListContainer* theme_list = nullptr; auto themes_page = page_scroll(theme_list); auto* theme_section = add_section(*theme_list,"testbed.ui_test.pages.appearance","testbed.ui_test.sections.appearance");
    ...
    auto theme_note = std::make_unique<UiLabel>(elysia::core::Rect{ 0,0,680,32 },0,ui_text_key("testbed.ui_test.appearance.note")); theme_note->set_visual_role(UiLabelVisualRole::Muted); theme_section->add_back(std::move(theme_note)); (void)tabs->add_tab(ui_text_key("testbed.ui_test.pages.appearance"),std::move(themes_page));

    UiListContainer* typography_list = nullptr; auto typography_page = page_scroll(typography_list); auto* typography_section = add_section(*typography_list,"testbed.ui_test.typography.title","testbed.ui_test.typography.description");
    struct TypographySample { const char* key; UiTypographyRole role; float height; };
    static constexpr std::array<TypographySample,7> typography_samples{{ { "testbed.ui_test.typography.sample_10",UiTypographyRole::Caption,26.0f },{ "testbed.ui_test.typography.sample_20",UiTypographyRole::ButtonCompact,36.0f },{ "testbed.ui_test.typography.sample_30",UiTypographyRole::Label,46.0f },{ "testbed.ui_test.typography.sample_40",UiTypographyRole::Heading,56.0f },{ "testbed.ui_test.typography.sample_50",UiTypographyRole::Subtitle,66.0f },{ "testbed.ui_test.typography.sample_60",UiTypographyRole::DialogTitle,76.0f },{ "testbed.ui_test.typography.sample_70",UiTypographyRole::Title,86.0f } }};
    ...
    (void)tabs->add_tab(ui_text_key("testbed.ui_test.typography.tab"),std::move(typography_page));
