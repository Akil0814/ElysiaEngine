...
{
    if (_configured
        || !factories.create_broad_phase_index
...
bool PhysicsService::apply_to(CollisionSystem& collision_system) const
{
    ...
    std::unique_ptr<IBroadPhaseIndex> broad_phase_index =
        _factories.create_broad_phase_index();
    ...
    if (!broad_phase_index || !discrete_detector || !continuous_detector || !response_strategy)
        return false;

    collision_system.set_broad_phase_index(std::move(broad_phase_index));
