...
struct PhysicsWorld::Impl {
 ...
 struct Mapping { CollisionTarget target{}; std::uint64_t retire_after = 0; };
 struct Snapshot { CollisionTarget target{}; std::optional<OneWayCollision> one_way; b2AABB bounds{}; b2Vec2 velocity{}; };
 struct Joint { b2JointId native{}; PhysicsObjectHandle first{},second{}; bool removed=false; };
 ...
 ~Impl();
 b2Vec2 to(elysia::core::Vector2 p) const { auto v=units.to_length(p); return {v.x,v.y}; }
 elysia::core::Vector2 from(b2Vec2 p) const { return units.from_length(elysia::core::Vector2{p.x,p.y}); }
 Object* get(PhysicsObjectHandle h) { auto i=objects.find(h.value); return i==objects.end()||i->second.removed?nullptr:&i->second; }
 const Object* get(PhysicsObjectHandle h) const { auto i=objects.find(h.value); return i==objects.end()||i->second.removed?nullptr:&i->second; }
 void enqueue(std::function<void()> f) { if(advancing) commands.push_back(std::move(f)); else f(); }
 void flush();
 void clear();
 ...
 void capture_debug();
 void query(const b2ShapeProxy&, b2Vec2 translation, const CollisionFilter&, bool sweep, bool ray, std::vector<CollisionQueryHit>&) const;
 std::optional<CollisionTarget> target(b2ShapeId id) const {
  auto it=mapping.find(b2StoreShapeId(id)); return it==mapping.end()?std::nullopt:std::optional(it->second.target);
 }
