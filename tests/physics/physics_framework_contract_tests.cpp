...
int main()
{
    for (float scale : {1.f, 32.f, 100.f, 1000.f})
    {
        ...
        for (float v : {-10000.f, -1.f, 0.f, 1.f, 2000.f})
        {
            ...
            require(near(u.from_squared(u.to_squared(v)), v, 0.002f),
                    "Squared quantities round trip");
        }
    }
    ...
    {
        Probe o;
        o.collider.shape = AabbShape{{0, 0, scale, scale}};
        o.definition.fixed_rotation = false;
        ...
        PhysicsWorld w(c);
        auto h = o.add(w);
        auto before = w.body_state(h);
        require(before && near(before->mass, 3), "Explicit mass");
        require(near(before->rotational_inertia / (scale * scale), 0.5f, 0.01f),
                "Inertia about center of mass scales with length squared");
        w.apply_impulse(h, {3 * scale, 0});
        require(near(w.body_state(h)->velocity.x / scale, 1), "Impulse conversion");
        float inertia = w.body_state(h)->rotational_inertia;
        // The returned inertia is about the center of mass.
        w.apply_torque(h, scale * scale);
        step(w);
        require(w.body_state(h)->angular_velocity > 0, "Positive torque turns clockwise");
