...
{
    Probe() : GameObject(elysia::core::DepthLayer::Item)
    {
        collider.shape = AabbShape{{0, 0, 20, 20}};
    }
    ...
    PhysicsObjectHandle add(PhysicsWorld &w)
    {
        return w.register_object(*this, definition, {&collider, 1});
    }
};
inline bool near(float a, float b, float e = 0.02f)
{
...
struct Events : ICollisionListener
{
    ...
    void on_collision_event(const CollisionEvent &e) override
    {
        events.push_back(e);
        if (callback)
            callback(e);
    }
    int count(CollisionEventPhase p) const
    {
        int n = 0;
        for (auto &e : events)
            n += e.phase == p;
